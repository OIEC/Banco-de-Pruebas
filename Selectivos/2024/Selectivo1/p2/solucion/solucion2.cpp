...
int main()
{
    ...
    bool alcanzable[N];                   // anota qué nodos ya han sido visitados. visited[k] = true implica que durante la busqueda hemos visitado la página (k+1), false es análogo.
    ...
    while (!q.empty())
    {
        ...
        { // si aún no hemos encontrado el camino mas corto, revisa si esta página es una página final (es decir, si no tiene vecinos).
            encontrado = true;
            menor_longitud = longitud;
        } // ya que el objetivo de la busqueda también es anotar qué páginas son alcanzables, no la rompemos después de encontrar el camino más corto.
